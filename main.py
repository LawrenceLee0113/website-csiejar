...
@app.route('/api/signup_auth', methods=["GET"])
def signup_auth():
    email = request.args.get("email")
    user_auth_uuid = request.args.get("user_auth_uuid")
    datetime2 = datetime.datetime.now()# now time
    datetime3 = datetime2 + relativedelta(hours=8)
    with open("static/data/signup_auth.json") as afile:
        adata = json.load(afile)
    name = adata["auth"][email]["name"]
    password = adata["auth"][email]["password"]
    auth_uuid = adata["auth"][email]["auth_uuid"]
    print(auth_uuid)
    print(user_auth_uuid)
    if user_auth_uuid == auth_uuid:
        ...
        del adata["auth"][email]
        with open("static/data/ID_and_google.json", "w") as file:
            json.dump(data, file)
        with open("static/data/signup_auth.json", "w") as afile:
            json.dump(adata, afile)
        print(login(our_id))
        print("user",login(our_id))#成功
...
def our_signup():
    #1. 跟驗證碼差不多吧 一樣傳一個a 連結去說  按此連結註冊ㄔㄥ按此連結註冊成功按此連結註冊成功
    #對
    
    #2. 那個驗證那個驗證碼 可以用 uuid 不用短 越長越好~
    #你說驗證碼給他們uuid? 還是只給連結？ 連結裡面包含 uuid + user_id
    #所以我要傳給他們七位數驗證碼+uuid? 不用驗證碼 uuid + user_id
    #kk 知道了 反正我就是會給你他們的uuid 跟user_id對不 黑 對 連結裡 就是csiejar.xyz/api/register_auth/?user_id= .... ?uuid= .....
    #這樣？
    
    
    #3. 然後 再創一個API 驗證 user_id to uuid 驗證後 傳fail or success Html(我做) 
    
    #4. (後端傳 user_id user_token?)
            ...
            if password.replace(" ", "") != "":
                if len(str(password)) >= 6:
                    if name.strip() != "":
                        ...
                        msg["Subject"] = "資工臭甲-註冊認證"
                        msg["From"] = "csiejarjar@gmail.com"
                        msg["To"] = email
                        html = """\
                                    <html>
                                    <body>
                                        嗨 """+name+""" 我們是資工臭甲 聽說你剛註冊我們的帳號?<br>
                                            <p>為了驗證你的電子郵件 請點擊下方連結<br>
                                        <a href="https://csiejar.xyz/api/signup_auth?email="""+email+"""&user_auth_uuid="""+auth_uuid+""""">點擊我來驗證你的電子郵件</a>
                                        <p>如果沒有註冊帳號 忽略此信件即可</p>
                        ...
                        with smtplib.SMTP_SSL("smtp.gmail.com", 465, context=context) as server:
                            server.login("csiejarjar@gmail.com", "mubmdpixdeoauscz")
                            server.sendmail(
                                "csiejarjar@gmail.com", email, msg.as_string()
                            )
                            print("Email成功寄出 收件者為"+name+"("+email+")")
                        with open("static/data/signup_auth.json","r") as afile:
                            auth_data = json.load(afile)
                            auth_data["auth"][email] ={
                                "auth_uuid":auth_uuid,
                                "password":password,
                                "name":name
                            }
                            with open("static/data/signup_auth.json","w") as afile:
                                json.dump(auth_data,afile)
